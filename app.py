...
DB_FILE = "fospuca_operativo1.db"


def init_db():
  conn = sqlite3.connect(DB_FILE)
  c = conn.cursor()
  # Añadimos la columna 'turno' a las tablas para soportar Turno Diurno y Nocturno
  c.execute("""
...
LISTA_ANALISTAS = [
    ...
    "GABRIELA ALEGRIA AZUAJE BOLCANES",
...
if menu == "📝 Registro de Datos":
  st.subheader("📝 Cargar o Actualizar Reporte")
  ...
  # --- LÓGICA AUTOMÁTICA DE TURNO SEGÚN LA HORA ---
  turno_automatico = "Diurno"
  try:
    t_parsed = datetime.datetime.strptime(report_time, "%H:%M:%S").time()
    limite_diurno = datetime.time(12, 0, 0)
    if t_parsed > limite_diurno:
  ...
  st.info(
      "Configura los valores de unidades (Total fijo predefinido por sede) y"
      " selecciona el analista correspondiente."
  )
  ...
  with st.form("fleet_form"):
    ...
    for sede in SEDES_DEFAULT:
      ...
      with c_tot:
        ...
        tot = st.number_input(
            f"Tot ({sede})",
            min_value=0,
            value=tot_predefinido,
            key=f"tot_{sede}",
            label_visibility="collapsed",
        )
      with c1:
        ruta = st.number_input(
            f"En Ruta ({sede})", min_value=0, value=7, key=f"ruta_{sede}"
        )
      with c2:
        sede_u = st.number_input(
            f"En Sede ({sede})", min_value=0, value=2, key=f"sed_{sede}"
        )
      with c3:
        taller = st.number_input(
            f"En Taller ({sede})", min_value=0, value=1, key=f"tall_{sede}"
        )
      with c4:
        gps = st.number_input(
            f"Falla GPS ({sede})", min_value=0, value=0, key=f"gps_{sede}"
        )
    ...
    if submitted_fleet:
      conn = sqlite3.connect(DB_FILE)
      c = conn.cursor()
      # Borramos registros previos de la misma fecha, semana y turno para evitar duplicados
      c.execute(
          "DELETE FROM fleet_status WHERE date = ? AND week = ? AND turno = ?",
          (str(report_date), report_week, turno_automatico),
      )
      for item in fleet_data:
        c.execute(
            """
                    INSERT INTO fleet_status (date, week, turno, sede, tot_unidades, und_ruta, und_sede, und_taller, fallas_gps, analista)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
            (
                item["date"],
      ...
      conn.close()
      st.success(
          f"¡Estatus de flota guardado exitosamente para el turno"
          f" {turno_automatico}!"
      )
  ...
  with col_t:
    with st.form("travel_form"):
      st.markdown(
          f"#### Registrar Unidad Viajera ({turno_automatico})"
      )
      ...
      if add_travel and t_unidad:
        ...
        c.execute(
            """
                    INSERT INTO travel_units (date, week, turno, unidad, conductor, desde, hasta, sede_resp)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
            (
                str(report_date),
  ...
  with col_n:
    with st.form("novedad_form"):
      ...
      if add_nov and nov_text:
        ...
        c.execute(
            """
                    INSERT INTO novedades (date, week, turno, novedad)
                    VALUES (?, ?, ?, ?)
                """,
            (str(report_date), report_week, turno_automatico, nov_text),
        )
        conn.commit()
        conn.close()
        st.success(
  ...
  if weeks_df.empty:
    ...
    # Permitir seleccionar turno en el dashboard en vivo también si se desea
    ...
    selected_turno_live = st.selectbox(
        "Seleccionar Turno",
        turnos_disponibles_live
        if turnos_disponibles_live
        else ["Diurno", "Nocturno"],
    )
  ...
  if df_all.empty:
    ...
    # Buscamos los turnos disponibles para la fecha seleccionada
