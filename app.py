...
if menu == "📝 Registro de Datos":
    ...
    if weeks_df.empty:
        ...
        with col_table1:
            ...
            display_fleet.columns = ["Sede", "Tot. U.", "Ruta", "En Sede", "Taller", "GPS"]
        ...
        def generate_chart_base64(val, total, title, color):
            ...
            wedges, texts = ax.pie([val, rest], colors=[color, '#e0e0e0'], startangle=90, 
                                   wedgeprops=dict(width=0.4, edgecolor='w'))
            ax.text(0, 0, f"{int((val/total*100) if total > 0 else 0)}%", ha='center', va='center', fontsize=10, fontweight='bold', color='#1b4d3e')
            plt.tight_layout()
            buf = io.BytesIO()
        ...
        for idx, row in df_fleet.iterrows():
            rows_html += f"<tr><td><strong>{row['sede']}</strong></td><td>{row['tot_unidades']}</td><td>{row['und_ruta']}</td><td>{row['und_sede']}</td><td>{row['und_taller']}</td><td>{row['fallas_gps']}</td><td>{row['analista']}</td></tr>"

        travel_html = ""
        if not df_travel.empty:
            for idx, row in df_travel.iterrows():
                travel_html += f"<tr><td>{row['unidad']}</td><td>{row['conductor']}</td><td>{row['desde']}</td><td>{row['hasta']}</td><td>{row['sede_resp']}</td></tr>"
        else:
            travel_html = "<tr><td colspan='5' style='text-align:center; color:#777;'>Sin unidades viajeras registradas.</td></tr>"
        ...
        if not df_nov.empty:
            ...
            nov_html = "<li style='color:#777; list-style:none;'>Sin novedades registradas para este periodo.</li>"
        ...
        <style>
            ...
            th {{ background-color: #1b4d3e; color: white; text-transform: uppercase; font-size: 8.5pt; }}
