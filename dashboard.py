...
def get_current_unit_assignment(flota: str, unidad: str) -> dict:
    ...
    try:
        ...
        return {} # Retorna vacío si no hay asignación
...
def create_asignacion_db(flota, fecha, unidad, conductor_ficha, telefono, ruta_nombre, hora_salida, hora_entrada):
    ...
    try:
        ...
        sql = """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
        cursor.execute(sql, (flota, fecha, unidad, conductor_ficha, telefono, ruta_nombre, hora_salida, hora_entrada))
        conn.commit()
...
def update_asignacion_db(asignacion_id, flota, fecha, unidad, conductor_ficha, telefono, ruta_nombre, hora_salida, hora_entrada):
    ...
    try:
        ...
        sql = """
            UPDATE asignacion 
            SET fecha = ?, unidad = ?, conductor_ficha = ?, telefono = ?, ruta_nombre = ?, hora_salida = ?, hora_entrada = ?
            WHERE id = ? AND flota = ?
        """
        cursor.execute(sql, (fecha, unidad, conductor_ficha, telefono, ruta_nombre, hora_salida, hora_entrada, asignacion_id, flota))
        conn.commit()
        get_all_asignaciones_db.clear()
        return True
    except Exception as e:
...
def display_asignacion_crud():
    """Muestra el formulario para Crear/Modificar/Eliminar Asignaciones, filtrado por FLOTA."""
    
    flota_a_usar = st.session_state.get('flota_seleccionada', 'Flota Principal')
    st.header(f"📝 Asignación de Unidades (Flota: {flota_a_usar})")
    
    st.button("⬅️ Volver al Menú", on_click=set_logistica_view, args=('menu',), key="volver_menu_asignacion_main")
    st.markdown("---")

    # Selector de Operación
    operation = st.radio(
        "¿Qué desea hacer?",
        ('Crear Nueva Asignación', 'Modificar Asignación Existente', 'Eliminar Asignación'),
        horizontal=True,
        key="asignacion_operation_radio"
    )
    ...
    # Si es Modificar o Eliminar, mostrar selector
    if operation != 'Crear Nueva Asignación':
        if not all_assignments:
            st.error("❌ No hay asignaciones registradas.")
            st.info("💡 **Solución:** Ve a 'Crear Nueva Asignación' para crear algunas asignaciones primero.")
            
            # Botón para crear datos de prueba
            if st.button("🚀 Crear Asignaciones de Prueba", help="Crea asignaciones automáticamente para probar"):
                create_sample_data(flota_a_usar)
                st.rerun()
            return
        
        # Crear opciones para seleccionar asignación
        assignment_options = []
        assignment_map = {}
        ...
        for assignment in all_assignments:
            ...
            display_option = f"[{assignment['fecha']}] {assignment['unidad']} - {nombre_conductor}"

            assignment_options.append(display_option)
            assignment_map[display_option] = assignment
        
        selected_assignment_key = st.selectbox(
            f"Seleccionar asignación a {operation.split()[0].lower()}:",
            ["Seleccione asignación..."] + assignment_options,
            key="asignacion_selector"
        )
        
        if selected_assignment_key != "Seleccione asignación...":
            current_assignment_data = assignment_map[selected_assignment_key]
            is_disabled_form = (operation == 'Eliminar Asignación')
            form_key = f"asignacion_form_mod_{current_assignment_data['id']}"
        else:
            is_disabled_form = True
            form_key = "asignacion_form_empty"
    
    # DATOS DE REFERENCIA PARA CREACIÓN
    fecha_seleccionada_default = str(date.today())
    
    if operation == 'Crear Nueva Asignación':
        # Obtener unidades y conductores disponibles
        ...
        # Para modificar/eliminar, obtener todas las unidades y conductores
        available_units = get_all_units_db(flota_a_usar)
        conductores_all = get_all_conductores_db(flota_a_usar)
        
        # Preparar mapa de conductores para modificación
        conductor_map = {}
        for ficha, data in conductores_all.items():
            nombre_completo = f"{data['nombre']} {data['apellido']}"
            telefono = data.get('telefono1', 'Sin Teléfono')
            display_name = f"{nombre_completo} | {telefono}"
            conductor_map[display_name] = {'ficha': ficha, 'telefono': telefono}
    ...
    with st.form(form_key, clear_on_submit=(operation == 'Crear Nueva Asignación')):
        st.subheader(f"Datos de la Asignación ({operation})")
        
        # Valores del formulario según operación
        if current_assignment_data:
            # Modificar o Eliminar - usar datos existentes
            fecha_value = datetime.strptime(current_assignment_data['fecha'], '%Y-%m-%d').date()
            unidad_value = current_assignment_data['unidad']
            conductor_ficha_value = current_assignment_data['conductor_ficha']
            telefono_value = current_assignment_data.get('telefono', '')
            ruta_value = current_assignment_data.get('ruta_nombre', '')
            hora_salida_value = current_assignment_data.get('hora_salida', '')
            hora_entrada_value = current_assignment_data.get('hora_entrada', '')
        else:
            # Crear - valores por defecto
            fecha_value = date.today()
            unidad_value = None
            conductor_ficha_value = None
            telefono_value = ""
            ruta_value = ""
            hora_salida_value = ""
            hora_entrada_value = ""
        ...
        with col2:
            if operation == 'Crear Nueva Asignación':
                ...
                # Para modificar/eliminar, mostrar como campo de texto
                unidad_seleccionada = st.text_input(
                    "Unidad", 
                    value=unidad_value or "", 
                    key="asignacion_unidad_field",
                    disabled=is_disabled_form
                )
        ...
        # Conductor
        if operation == 'Crear Nueva Asignación':
            # Para crear, mostrar selectbox de conductores disponibles
            ...
            # Para modificar/eliminar, mostrar campos de texto
            col_c1, col_c2 = st.columns(2)
            with col_c1:
                conductor_ficha_input = st.text_input(
                    "Ficha del Conductor", 
                    value=conductor_ficha_value or "", 
                    key="asignacion_conductor_ficha_field",
                    disabled=is_disabled_form
                )
            with col_c2:
                telefono_input = st.text_input(
                    "Teléfono", 
                    value=telefono_value or "", 
                    key="asignacion_telefono_field",
                    disabled=is_disabled_form
                )
        ...
        if operation == 'Crear Nueva Asignación':
            ...
            ruta_seleccionada = st.text_input(
                "Ruta", 
                value=ruta_value or "", 
                key="asignacion_ruta_field",
                disabled=is_disabled_form
        ...
        # Botón de submit
        enable_submit = operation == 'Crear Nueva Asignación' or (current_assignment_data is not None)
        ...
        if submitted:
            ...
            elif operation == 'Modificar Asignación Existente':
                # Actualizar la asignación
                if update_asignacion_db(
                    current_assignment_data['id'],
                    flota_a_usar,
                    str(fecha_asignacion),
                    unidad_seleccionada,
                    conductor_ficha_input,
                    telefono_input,
                    ruta_seleccionada,
                    hora_salida.strip(),
                    hora_entrada.strip()
            ...
            elif operation == 'Eliminar Asignación':
                # Eliminar la asignación
                if delete_asignacion_db(current_assignment_data['id'], flota_a_usar):
...
COLOR_FUERA_DE_PERIMETRO = "#7627F5" # ¡NUEVO COLOR PARA ESTADO FUERA DE PERÍMETRO!
...
def get_card_style(ignicion_status, speed):
    ...
    elif "Fuera de Perímetro" in ignicion_status: # ¡NUEVO ESTADO!
        bg_color = "#7627F5"
        text_color = "white"
...
def obtener_datos_unidades(nombre_flota: str, config: Dict[str, Any], gps_min_encendida: int, gps_min_apagada: int):
    ...
    # 🚨 CARGA DE PERÍMETROS DESDE ARCHIVOS JSON 🚨
    ...
    # Usar los perímetros cargados con la nueva lógica
    if nombre_flota in PERIMETROS_CARGADOS:
        ...
        try:
            ...
            print(f"❌ Error al cargar perímetro de {nombre_flota}: {e}")
    else:
        print(f"ℹ️ No se encontró archivo de perímetro para {nombre_flota}: {archivo_perimetro_path}")
    ...
    try:
        ...
        for unidad in lista_unidades:
            ...
            if es_falla_gps:
                ...
                # 4. ¿Está FUERA DE PERÍMETRO? (Solo si NO está en otras ubicaciones críticas)
                en_fuera_perimetro = False
                if coordenadas_perimetro and nombre_flota in PERIMETROS_CARGADOS and not (en_vertedero or en_sede or en_resguardo_secundario):
                    # Usar la nueva función de verificación con Shapely
                    perimetros_encontrados = verificar_coordenada_en_perimetro(lat, lon, {nombre_flota: PERIMETROS_CARGADOS[nombre_flota]})
                    if not perimetros_encontrados:
                        en_fuera_perimetro = True
                elif coordenadas_perimetro and not (en_vertedero or en_sede or en_resguardo_secundario):
                    # Fallback: usar la función antigua si no hay Shapely disponible
                    if not es_punto_dentro_perimetro(lat, lon, coordenadas_perimetro):
                        en_fuera_perimetro = True
                
                # Aplicar el estado de "Fuera de Perímetro" si es aplicable
                if en_fuera_perimetro and coordenadas_perimetro:
...
with st.sidebar:
    ...
    # INICIO DEL EXPANDER DE FILTROS
     if st.session_state['flota_seleccionada']:
        ...
        with st.sidebar.expander("🚚 Logística", expanded=False):
    # EXPANDER ANIDADO: Data
    
            with st.expander("📊 Data", expanded=False):
            # BOTONES dentro de Data
            ...
            st.button(
                ...
                args=('asignacion_crud',),
                key="btn_asignacion_sidebar"
            )
