            ...
            for row_index, row_data in enumerate(rows):
                ...
                for col_index, row_tuple in enumerate(row_data.iterrows()):
                    with cols[col_index]:
                        ...
                        st.markdown(f'<div style="{card_style}"><p style="font-size: 0.8em; margin-top: 0px; opacity: 1.1; text-align: center; margin-bottom: 0px; font-weight: bold;">{estado_display}</p></div>', unsafe_allow_html=True)

                        last_report_dt = datetime.strptime(last_report_display, "%b %d %Y %I:%M%p")
                        fecha_formateada = last_report_dt.strftime("%d/%m/%y %H:%M")
                        st.markdown(f'<span style="font-size: 0.9em;">🕒 {fecha_formateada}</span>', unsafe_allow_html=True)
