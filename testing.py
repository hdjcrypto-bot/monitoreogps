...
if 'ultima_velocidad_unidades' not in st.session_state:
    st.session_state.ultima_velocidad_unidades = {}


def enviar_alerta_n8n(unidad, velocidad, limite, latitud, longitud):
    # PEGA AQUÍ LA URL QUE COPIASTE EN EL PASO ANTERIOR
    WEBHOOK_URL = "http://147.93.2.250:5678/webhook-test/a28852ed-3635-4fa1-8f12-01109ac76eb1"
    ...
    try:
        # Enviamos la data a la nube de n8n
...
def construir_status_con_emojis(row_data, is_en_perimetro, estado_display=None):
    ...
    if estado_display is None:
        ...
        if row_data['ES_FALLA_GPS_FLAG']:
            ...
            return '<span style="color: red;">🛑</span> EXCESO 🚨'
        elif velocidad >= SPEED_THRESHOLD_KPH:
            return '<span style="color: orange;">🛑</span> Alerta ⚠️'
        else:
...
# 3. Aplicar el estado y estilo si es Falla GPS
    if es_falla_gps:
        ...
        unidad_data['CARD_STYLE_OVERRIDE'] = f"background-color: {COLOR_FALLA_GPS}; padding: 4px 10px; border-radius: 5px; color: black; margin-bottom: 0px;"
...
def get_fallback_data(error_type="Conexión Fallida"):
    ...
    return pd.DataFrame([{
        ...
        "CARD_STYLE": "background-color: #D32F2F; padding: 4px 10px; border-radius: 5px; color: white; margin-bottom: 0px;",
        ...
        for unidad in lista_unidades:
            ...
            if es_falla_gps:
                ...
                if en_vertedero:
                    ...
                    if en_sede:
                        estado_final_display = "Resguardo 🛡️";
                        color_fondo = "#337ab7"
                        color_texto = "white"
                        
                    elif en_resguardo_secundario:
                        estado_final_display = "Resguardo (FS) 🛡️";
                        color_fondo = "#191452"
                ...
                if en_fuera_perimetro:
                    ...
                    estado_final_display = "F Perímetro 🌐"
                    color_fondo = "#7627F5"
                    color_texto = "white"
                #else:
                ...
                card_style = f"background-color: {color_fondo}; padding: 4px 10px; border-radius: 5px; color: {color_texto}; margin-bottom: 0px;"
...
while True:
    ...
    with placeholder_main_content.container():
        ...
        st.markdown(f"## Rastreo GPS - Flota {flota_a_usar}")
        
        #st.markdown(f"<h4 id='main-title'>Ubicacion Sede - Flota {flota_a_usar}</h4>", unsafe_allow_html=True)
                ...
                with card_col:
                    ...
                    if selected_row['ES_FALLA_GPS_FLAG']:
                        ...
                        if stop_duration > STOP_THRESHOLD_MINUTES and velocidad_float < 1.0 and is_out_of_hq_status:
                            parada_display = f"Parada Larga 🛑"
                            card_style = "background-color: #FFC107; padding: 2px 4px; border-radius: 5px; color: black; margin-bottom: 0px;"
                    ...
                    st.markdown(
                        ...
                        f'<span style="background-color: rgba(0,0,0,0.3); color:{color_Unidad}; padding: 5px 10px; border-radius: 5px; font-size: 1.5em; font-weight: 900;">'
                ...
                for col_index, row_tuple in enumerate(row_data.iterrows()):
                    with cols[col_index]:
                        ...
                        if row['ES_FALLA_GPS_FLAG']:
                            ...
                            if stop_duration > STOP_THRESHOLD_MINUTES and velocidad_float < 1.0 and is_out_of_hq_status:
                                parada_display = f"Parada Larga 🛑"
                                card_style = "background-color: #FFC107; padding: 4px 10px; border-radius: 5px; color: black; margin-bottom: 0px;"
                                estado_display = parada_display
                                color_velocidad = "black"
                        ...
                        st.markdown(f'<div style="{card_style}"><p style="font-size: 0.8em; margin-top: 0px; opacity: 2; text-align: center; margin-bottom: 0px; font-weight: bold;">{estado_display}</p></div>', unsafe_allow_html=True)

                        from datetime import datetime

                        last_report_dt = datetime.strptime(last_report_display, "%b %d %Y %I:%M%p")

                        hoy = datetime.now().date()
                        fecha_reporte = last_report_dt.date()
                        
                        color_texto = "#FF0000" if fecha_reporte != hoy else "#FFFFFF"
                        fecha_formateada = last_report_dt.strftime("%d/%m/%y %H:%M")

                        
                        st.markdown(
                            f'<div style="text-align: center; font-size: 0.8em; color: {color_texto};font-weight: bold;">🕒 {fecha_formateada}</div>', 
                            unsafe_allow_html=True
                            )
                        ...
                        with col1:
                            st.markdown(
                                ...
                                <div style="display: flex; justify-content: center; align-items: center; height: 100%;">
                                    <a href="{link_google}" target="_blank" style="text-decoration: none; color: inherit;">
                                        <span class="boton-unidad" style="background-color: rgba(0,0,0,0.3); padding: 5px 15px; border-radius: 5px; 
                                                 font-size: 1.2em; font-weight: 900; white-space: nowrap; cursor: pointer; display: inline-block;">
                                            {nombre_unidad_display[:8]}
                                        </span>
                                    </a>
                                </div>
                                ''',
                            ...
                            #st.markdown(
                                ...
                                #f'<span style="background-color: rgba(0,0,0,0.3); padding: 5px 15px; border-radius: 5px; '
                                #f'font-size: 1.2em; font-weight: 900; white-space: nowrap;">'
                                #f'{nombre_unidad_display[:8]}'
                                #f'</span></div>',
                            #unsafe_allow_html=True
                            #)
                        with col2:
                            st.markdown(
                                ...
                                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; line-height: 1.1;">
                                    <div style="font-size: 1.6em; font-weight: 900; color: {final_text_color};">
                                        {velocidad_formateada}
                                    </div>
                                    <div style="font-size: 0.em; font-weight: 700; color: {final_text_color}; text-transform: uppercase;">
                                        Km
                                    </div>
                                </div>
                            """,
                        ...
                        #st.markdown(
                            #f"""
                            #<a href="{link_google}" target="_blank" style="text-decoration: none;">
                                #<div style="
                                    #display: flex;
                         ...
                         #with col2:
                            ...
                            # 🆕 STATUS CON EMOJIS COLORIDOS
                               ...
                               #  link_google = f"https://www.google.com/maps?q={row['LATITUD']},{row['LONGITUD']}"
                                ...
                                # st.markdown(f"📍 [Google Maps](https://www.google.com/maps?q={row['LATITUD']},{row['LONGITUD']})")
                               #  button_uuid = str(row['UNIDAD']).replace(" ", "_") # ID único para cada unidad
                               #  copy_js = f"""
                                #     <script>
                                #     function copyToClipboard_{button_uuid}() {{
                                #         const text = "{link_google}";
                                #         navigator.clipboard.writeText(text).then(() => {{
                                #         alert("¡Link copiado al portapapeles!");
                                 #        }});
                                  #   }}
                                  #   </script>
                                  #   <button onclick="copyToClipboard_{button_uuid}()" style="
                                    #     width: 100%;
                                    ...
                                    #     margin-top: 5px;
                                     ...
                                     #    align-items: center;
                                       #  justify-content: center;
                                      #   gap: 8px;
                                     #" onmouseover="this.style.borderColor='#f0e4e4'; this.style.color='#f0e4e4'" 
