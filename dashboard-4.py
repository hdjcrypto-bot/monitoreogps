        ...
        if not unidades_en_alerta_speed.empty:
            ...
            if not alertas_visuales.empty:
                ...
                for _, row in alertas_visuales.head(5).iterrows():
                    nombre_unidad = row['UNIDAD'].split('-')[0]
                    velocidad_formateada = f"{row['VELOCIDAD']:.1f} Km/h"
                    mensaje_alerta_speed += (f"**{nombre_unidad}** (ALERTA a {velocidad_formateada}):\n---\n")
        ...
        if unidad_a_ubicar_id and not is_fallback:
            ...
            if not df_unidad_seleccionada.empty:
                ...
                with card_col:
                    ...
                    is_out_of_hq_status = not (selected_row['EN_SEDE_FLAG'] or selected_row['EN_RESGUARDO_SECUNDARIO_FLAG'] or selected_row['EN_VERTEDERO_FLAG'] or selected_row['ES_FALLA_GPS_FLAG'])
                    ...
                    with card_col:
                        unit_id_to_lookup = selected_row['UNIT_ID']
                        datos_unidad = datos_flota_conductor.get(unit_id_to_lookup, {})
                        conductor = datos_unidad.get('conductor', 'No Asignado 👤')    
                        ruta = datos_unidad.get('ruta', 'No Asignada 🛣️')
                        telefono = datos_unidad.get('telefono', 'No Disponible 📞')
                        Salida = datos_unidad.get('holras', 'No Salio a Ruta')
                        Entrada = datos_unidad.get('holrae', 'No Disponible')


                    st.caption(f"Ruta: **{ruta}**")
                    col1, col2 = st.columns(2)
                    with col1:
                        st.caption(f"Conductor: **{conductor}**")
                    with col2:
                        st.caption(f"Telef: **{telefono}**")
